...
date: "(updated: `r Sys.Date()` by MNM)"
output: 
  tufte::tufte_html:
...
Find your homework repo link, clone it in RStudio and open the R Markdown document.
Knit the document to make sure it compiles without errors, it should be largely empty to start.
...
We have an opportunity here for dividing tasks.  For example, exercises 1 and 2 are on two different datasets, so two different people can work on them at the same time. Be sure to **pull then push** often, you don't need to knit every time. Resolve conflicts often to keep them small. If you work on different lines, the GIT should be able to put them together.
Look at the final .md file. **That** is what will be graded.
...
```
You can load them by running the following. If you're also running code in real time, you need to load them in your Console:
...
6.  Add a country variable to the Denny's dataset and set all observations equal to `"United States"`. Remember, you can use the `mutate` function for adding a variable. Make sure to save the result of this (as `dn`) so that the stored data frame contains the new variable going forward.
...
7.  Find the La Quinta locations that are outside the US, and figure out which country they are in.
    This might require some googling or looking at the original website.
...
29 points:
-  2 points per exercise for correctness of code and adequate prose answers/description (where specified)
-  5 points for adequately commented commits
