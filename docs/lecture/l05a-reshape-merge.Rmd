...
date: "2021-09-06 (updated: `r Sys.Date()` by MNM)"
output:
...
Here's the [R code in this lecture](l05a/l05a-reshape-merge.R)

Here's the different data sets:
 [customers](l05a/data/sales/customers.csv),
 [prices](l05a/data/sales/prices.csv),
 [biden](l05a/data/trump/biden.csv),
 [professions](l05a/data/scientists/professions.csv),
 [dates](l05a/data/scientists/dates.csv),
 [works](l05a/data/scientists/works.csv),
 [enrollment](l05a/data/students/enrolment.csv),
 [survey](l05a/data/students/survey.csv)
...
* `nest()` and `unnest()` let you embed or expand list columns in a data frame.  .alert[This can be extremely powerful.] But it is best illustrated with an example...in a couple weeks.
...
For example, each plane is uniquely identified by its `tailnum`, but to identify an observation in `weather` you need five variables: `year`, `month`, `day`, `hour`, and `origin`.
...
Tidy data sets will obey [various .alert[normal forms]](https://en.wikipedia.org/wiki/Database_normalization).  The most basic is that there exists a .alert[primary key] obeying a uniqueness and existence constraint.
