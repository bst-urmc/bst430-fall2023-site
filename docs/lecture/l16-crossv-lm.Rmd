...
date: "2021-11-07 (updated: `r Sys.Date()` by MNM)"
output:
...
For a predictive goal, probably should use some method that does shrinkage and basis expansion.  For inference, need to consider putative causal models.

---


- Use `broom::glance()` function to get the estimated standard deviation (`sigma`), $R^2$, and information criteria. 
    ```{r}
    glance(lmout)
    ```
...
### What do we lose when violated?
  ...
  1. **Independence** violated - Linear regression line is unbiased, but standard 
     errors can be badly off.
  2. **Linearity** violated - Linear regression line does not pick up actual 
